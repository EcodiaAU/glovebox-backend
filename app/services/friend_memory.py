...
async def resolve_friend(
    friend_id: Optional[str], email: Optional[str] = None
) -> Tuple[Optional[str], str]:
    """Resolve (friend_name, memory_prelude) for a person against the shared
    Ecosphere project. Returns (None, "") when no lookup key, ECOSPHERE_* unset,
    or the account/name is absent. Never raises - the guide must survive a memory
    outage by degrading to the neutral persona.

    The name (e.g. "Chubz") lives in ecosphere_accounts, keyed by owner_id = the
    ecosphere-LOCAL user id. But the friend_id a federated app carries is the
    canonical Friend id from whichever IdP it federated into, which does NOT
    equal that local id (Studio stamped its own project id; Glovebox/Locals
    carry the eionabt id). So owner_id = friend_id only matches for same-project
    logins. Email is the one key that is stable across every project, so we match
    on owner_id = friend_id OR email = the signed-in email; email is what makes
    the name resolve for the federated apps."""
    ...
    try:
        async with httpx.AsyncClient(timeout=_HTTP_TIMEOUT_S) as client:
            # Match on owner_id = friend_id OR email = the signed-in email. Order
            # by friend_name so a row that actually carries a name wins over a
            # bare auto-created shell if both keys hit different rows.
            or_terms = []
            if friend_id:
                or_terms.append(f"owner_id.eq.{friend_id}")
            if email:
                or_terms.append(f"email.eq.{email}")
            acct_params = {
                "select": "id,friend_name",
                "order": "friend_name.desc.nullslast",
                "limit": "1",
            }
            if len(or_terms) == 1:
                # Single key: a plain filter (owner_id / email) is exact.
                k, v = or_terms[0].split(".eq.", 1)
                acct_params[k] = f"eq.{v}"
            else:
                acct_params["or"] = f"({','.join(or_terms)})"
